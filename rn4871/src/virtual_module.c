...
void virtualModuleReceiveData(VIRTUAL_MODULE *virtualModule,
      ...
      switch (command) {
        ...
        case CMD_DUMP_INFOS: {
          snprintf(virtualModule->_global_buffer, BUFFER_MAX_LEN,
                   "BTA=%s\r\nName=%s\r\nConnected=no\r\nAuthen=0\r\nFeatures="
                   "0000\r\nServices=%X\r\nCMD> ",
                   virtualModule->macAddress, virtualModule->moduleName,
                   virtualModule->services);
          break;
        }
...
void virtualModuleSendData(VIRTUAL_MODULE *virtualModule, char *dataToSend,
...
RN4871_CODE_RETURN virtualModuleConnect(VIRTUAL_MODULE *virtualModule,
                                        RN4871_MODULE *dev) {
...
RN4871_CODE_RETURN virtualModuleStream(VIRTUAL_MODULE *virtualModule,
                                       RN4871_MODULE *dev) {
  assert((NULL != virtualModule) || (NULL != dev));
...
RN4871_CODE_RETURN virtualModuleDisconnect(VIRTUAL_MODULE *virtualModule,
                                           RN4871_MODULE *dev) {
  assert((NULL != virtualModule) || (NULL != dev));
